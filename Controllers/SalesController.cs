...
namespace AlphaPlusAPI.Controllers
{
    ...
    public class SalesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetNextInvoiceNumber()
        {
            try
            {
                ...
                _logger.LogInformation($"✅ Next Invoice Generated: No={nextInvoiceNo}");
    ...
    VALUES (
        ...
        '', 0, 0, 0, '', @SalesmanName, @CustomerName
                ...
                {
                    _logger.LogInformation($"💳 Inserting {request.Payments.Count} payments into Invoice_Payment...");
                ...
                _logger.LogInformation($"🏁 SALE COMPLETED - Inv_ID: {newInvId}, InvoiceNo: {request.InvoiceNo}, Total: {request.GrandTotal}");
        ...
        private async Task DeductStock(SqlConnection connection, SqlTransaction transaction, List<SaleProductRequest> products)
        {
            try
            {
                ...
                foreach (var product in products)
                {
                    ...
                    var currentStock = await connection.ExecuteScalarAsync<double?>(checkStockQuery, new { product.ProductId, Barcode = barcode }, transaction);
                    ...
                    int stockRows = await connection.ExecuteAsync(stockUpdateQuery, new { product.Quantity, product.ProductId, Barcode = barcode }, transaction);
