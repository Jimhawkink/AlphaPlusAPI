    ...
    public class SalesController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetNextInvoiceNumber()
        {
            try
            {
                ...
                // Get next invoice number using the same logic as GenerateInvoiceNo
                const string query = @"
                ...
                // ✅ Property names must be camelCase to match Kotlin InvoiceNumberData
                return Ok(new
                {
                    success = true,
        ...
        public async Task<IActionResult> SaveSale([FromBody] SaveSaleRequest request)
        {
            ...
            // ✅ CRITICAL VALIDATION: Check InvId
            if (request.InvId <= 0)
            {
            ...
            try
            {
                ...
                // Use the InvId from request (generated by client)
                ...
                await connection.ExecuteAsync("SET IDENTITY_INSERT InvoiceInfo ON", transaction: transaction);
                _logger.LogInformation("🔓 Identity insert enabled");
                ...
                _logger.LogInformation($"✅ InvoiceInfo inserted - Rows affected: {invoiceRows}");
                ...
                await connection.ExecuteAsync("SET IDENTITY_INSERT InvoiceInfo OFF", transaction: transaction);
                _logger.LogInformation("🔒 Identity insert disabled");
        ...
        private async Task DeductStock(SqlConnection connection, SqlTransaction transaction, List<SaleProductRequest> products)
        {
            try
            {
                ...
                foreach (var product in products)
                {
                    ...
                    {
                        // ❌ Stock record doesn't exist - throw error
                    ...
                    {
                        // This should NEVER happen after our checks above
